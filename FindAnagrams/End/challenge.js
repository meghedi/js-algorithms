...
const hasSmaeLetterCount = (word1, word2)=>{
    const word1COunt =countOccurrences(word1.split(''));
    const word2Count = countOccurrences(word2.split(''));

    return Object.keys(word1COunt).every(letter=> word1COunt[letter] === word2Count[letter]) 
        && Object.keys(word1COunt).length === Object.keys(word2Count).length;
}

const findAnagrams = (word, allWords) => {
    return allWords.filter(entry=> hasSmaeLetterCount(word, entry)).filter(anagram=> anagram!=word);
   /* return allWords
